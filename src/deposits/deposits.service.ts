...
export class DepositsService {
    ...
    async listDeposits(
        ...
        */
        ...
        const [rawDeposits, total] = await this.prisma.$transaction([
        ...
        const deposits = rawDeposits.map((d) => ({
            ...
            bonus_amount: realFormat(d.bonus_amount ?? 0),
            lead: d.lead ? {
                id:    d.lead.id,
                img:   d.lead.img,
        ...
        if (context === 'affiliate') {
            user = user_id && rawDeposits.length > 0
                ? rawDeposits[0].affiliate
                : null;
        } else if (context === 'lead') {
            const raw = user_id && rawDeposits.length > 0
                ? rawDeposits[0].lead
                : null;
